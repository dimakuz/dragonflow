...
class TestSfcApp(test_base.DFTestBase):
    ...
    def test_ping_pong(self):
        ...
        def send_initial_packet(*_):
            src_lport = self.nb_api.lport.get(self.src_port.port.port_id)
            dst_lport = self.nb_api.lport.get(self.dst_port.port.port_id)
        ...
        port_policies = {
            pp1_ingress_key: app_testing_objects.PortPolicy(
                rules=[
                    app_testing_objects.PortPolicyRule(
                        app_testing_objects.RyuIPv4Filter(),
                        actions=[
                            app_testing_objects.SendAction(
                                self.subnet.subnet_id,
