...
class FlowClassifier(NbDbObjectWithUniqueKey):
...
class PortChain(NbDbObject):
    ...
    def get_port_pair_group_ids(self):
        return self.inner_obj.get('port_pair_group_ids')

    def get_flow_classifier_ids(self):
        return self.inner_obj.get('flow_classifier_ids')

    def _get_mpls_label(self, fc_idx, ppg_idx):
        chain_idx = self.get_service_path_id()

        # FIXME overflows, allocate smarter
        return fc_idx | (ppg_idx << 5) | (chain_idx << 10)

    def get_mpls_ingress_label(self, fc_idx, ppg_idx):
        return self._get_mpls_label(fc_idx, ppg_idx)

    def get_mpls_egress_label(self, fc_idx, ppg_idx):
        return self._get_mpls_label(fc_idx, ppg_idx + 1)
